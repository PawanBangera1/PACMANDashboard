...
const Dashboardheader = ({ compact = false }: DashboardHeaderProps) => {
  ...
  const [selectedApp, setSelectedApp] = useState("App 5");
  ...
  // 1. Back functionality logic
  const handleBackToAll = (e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent opening the dropdown when clicking the text
    setSelectedApp("");
    setIsAppOpen(false);
  };
  ...
  if (compact) {
    return (
      <div className="relative mx-auto mb-4 rounded-sm bg-white shadow-lg">
        <div className="flex items-center justify-between gap-3 px-3 py-2 md:px-6 md:py-3">
          <div className="flex items-center gap-3 cursor-pointer" onClick={() => setIsAppOpen(!isAppOpen)}>
  ...
  return (
    <div className="relative mx-auto mb-4 bg-white shadow-lg rounded-sm">
      <div className="grid grid-cols-1 md:grid-cols-[1fr_1.2fr_auto] gap-0 items-stretch">
        
        {/* Breadcrumb Application Selector */}
        <div className="relative flex items-center gap-3 px-4 py-2 cursor-pointer" onClick={() => setIsAppOpen(!isAppOpen)}>
        ...
        {/* Stats Section */}
        <div className="flex items-center justify-center gap-6 px-5 py-2 text-xs border-l border-slate-100">
