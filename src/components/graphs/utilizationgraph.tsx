...
// Expanded data set to create a long zig-zag "hill" effect
...
const UtilizationGraph = ({ detail = false }) => {
  ...
  return (
    <div className="flex flex-col gpa-8 h-full px-6 pt-4 pb-4 bg-white">
      
      {/* CPU Hill Chart */}
      <div className="flex items-start gap-8 h-[50px] mt-2">
        <p className="text-[10px] font-bold text-gray-500 mb-1 absolute -left-1">CPU</p>
        ...
        <div className="flex-1 relative">
          ...
          <ResponsiveContainer width="100%" height={chartHeight}>
            <AreaChart data={utilizationDetailSeries} margin={{ top: 20, right: 0, bottom: 0, left: 0 }}>
              ...
              <XAxis hide />
              <Area type="monotone" dataKey="cpu" stroke="#E91E63" strokeWidth={2} fill="url(#colorCpu)" isAnimationActive={false} dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      {/* I/O Hill Chart */}
      <div className="flex items-start gap-4 h-[50px] mt-10">
        <div className="flex flex-col justify-between text-[10px] text-gray-400 h-[65px] mt-5 w-12 text-right">
          <span>400 kb/s</span>
          <span>200 kb/s</span>
          <span>0 kb/s</span>
        ...
        <div className="flex-1 relative">
          <p className="text-[10px] font-bold text-gray-500 mb-1 uppercase tracking-widest absolute -left-1">I/O</p>
          <ResponsiveContainer width="100%" height={chartHeight}>
            <AreaChart data={utilizationDetailSeries} margin={{ top: 20, right: 0, bottom: 0, left: 0 }}>
              <defs>
              ...
              <XAxis hide />
              <Area type="monotone" dataKey="io1" stroke="#8BC34A" strokeWidth={1.5} fill="url(#colorIo1)" isAnimationActive={false} dot={false} />
              <Area type="monotone" dataKey="io2" stroke="#9C27B0" strokeWidth={1.5} fill="url(#colorIo2)" isAnimationActive={false} dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
        ...
        <div className="flex-1 relative">
          ...
          <ResponsiveContainer width="100%" height={chartHeight}>
            <AreaChart data={utilizationDetailSeries} margin={{ top: 20, right: 0, bottom: 0, left: 0 }}>
              ...
              <XAxis hide />
              <Area type="monotone" dataKey="disk1" stroke="#FF9800" strokeWidth={1.5} fill="url(#colorDisk1)" isAnimationActive={false} dot={false} />
              <Area type="monotone" dataKey="disk2" stroke="#0288D1" strokeWidth={1.5} fill="url(#colorDisk2)" isAnimationActive={false} dot={false} />
            </AreaChart>
          </ResponsiveContainer>
        </div>
      </div>
